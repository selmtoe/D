...
WebXRの`immersive-vr`に対応したヘッドセット内ブラウザーで対局画面を開くと、右下に「VRで遊ぶ」が表示されます。左右どちらのコントローラーでも、レーザーをカードや3D操作パネルへ合わせて引き金を押してください。手札の選択、提出、パス、A奪い、7渡し、10捨て、Qボンバー、K回収、Joker宣言、観戦視点の切替をヘッドセット内で操作できます。ブラインド札を含む提出は、誤操作防止のため提出ボタンをもう一度引いて確定します。

WebXRはHTTPSまたはlocalhost上でのみ利用できます。非対応端末ではVRボタンを表示せず、従来のPC／スマホ操作だけが動作します。
