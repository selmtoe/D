...
describe("PWA update safety", () => {
  it("applies a waiting update only from an idle room-free screen", () => {
    expect(canApplyPwaUpdate("AUTHENTICATING", false, false)).toBe(true);
    expect(canApplyPwaUpdate("ENTRANCE", false, false)).toBe(true);
    ...
    expect(canApplyPwaUpdate("ENTRANCE", true, false)).toBe(false);
